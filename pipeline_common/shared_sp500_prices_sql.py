...
def _connect_shared_prices_postgres():
    ...
    if url:
        return _PostgresConnectionProxy(psycopg.connect(url, prepare_threshold=None, autocommit=True))
    config = shared_prices_supabase_config()
    if not config:
        raise RuntimeError(
            "Set KEUMJ_SP500_SUPABASE_DATABASE_URL or KEUMJ_SP500_SUPABASE_HOST/USER/PASSWORD."
        )
    return _PostgresConnectionProxy(psycopg.connect(**config, prepare_threshold=None, autocommit=True))
